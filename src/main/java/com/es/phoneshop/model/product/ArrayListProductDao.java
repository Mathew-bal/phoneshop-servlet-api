...
public class ArrayListProductDao implements ProductDao {
    ...
    public interface ProductReader<T>{
        T apply() throws ProductNotFoundException;
    }

    public <T> T readProductsSafe(ProductReader<T> getProd) throws ProductNotFoundException{
        readLock.lock();
        try {
            return getProd.apply();
        }finally {
            readLock.unlock();
        }
    }
    ...
    public boolean writeProductsSafe(ProductWriter writeProd){
        writeLock.lock();
        try {
            return writeProd.apply();
        }finally {
            writeLock.unlock();
        }
    }
    ...
    public Product getProduct(Long id) throws ProductNotFoundException {
        return readProductsSafe(() -> products.stream().
    ...
    public List<Product> filterProducts() throws ProductNotFoundException {
        return readProductsSafe(() -> {
    ...
    public boolean save(Product product){
        return writeProductsSafe(() ->{
            if (product.getId() == null){
                product.setId(maxId++);
                return products.add(product);
            }else {
                // Boolean value instead of comparing old and new lists
                AtomicBoolean changed = new AtomicBoolean(false);
                //List<Product> old = new ArrayList<>(products);

                products = products.stream().map((pr) ->
                {
